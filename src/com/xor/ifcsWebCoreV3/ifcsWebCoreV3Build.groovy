package com.xor.xorWebCoreV3

class xorWebCoreV3Build implements Serializable {
   ...
   // pipeline global properties
    ...
    xorWebCoreV3Build(steps, env, params, scm, currentBuild) { 
        this.steps = steps
	this.env = env
        this.params = params
        this.scm = scm
    ...
    def cascade(xorRepos) {
        for (int i = 0; i < xorRepos.size(); i++) {  // trigger xor-server job and deploy on respective client environment
             steps.build(job: "${orgName}/${xorRepos[i]}"])
        }			  
    }
