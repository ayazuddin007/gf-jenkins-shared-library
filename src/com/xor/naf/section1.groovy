package com.xor.naf
import com.xor.features.sonarqube
...
class section1 implements Serializable {
   ...
   section1(steps, env, params, scm, currentBuild) {
    ...
    static final String registry_id = "nafleet-acs-docker.us-east-1.artifactory.wexapps.com"
    static final String registry_id_maven = "nafleet-acs-maven"
	...
	def version_pom = ''   	
    	...
    	if ("${env.artifactId_pom}" == "xorFramework") {
	      env.artifactId_pom = "xor-batch-framework"
        }	
        steps.echo "${env.artifactId_pom}"

	def maven_settings = ''
        if ("${env.artifactId_pom}" == "xor-web-ols-ui" || "${env.artifactId_pom}" == "xor-web-ajaxswing-angularui") {
