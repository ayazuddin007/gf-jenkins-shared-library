...
class section2 implements Serializable {
    ...
    static final String cr_groupId = 'com.wexinc.gf.ifcs.run.app.wfe'
    public static namespaces = []	
		
    def gitCheckout() {  
    	def (githubRepoName, githubRepoBranch) = "${params.controlRepos}".tokenize( '/' )
	steps.git(branch: "${githubRepoBranch}", credentialsId: "${params.credentialsId}", url: "https://github.com/wexinc/${githubRepoName}.git")    
    }
    ...
    def cascade(EnvRepo) {  
	 	...
	 	steps.build(job: "${params.orgName}/${EnvRepo[j]}", parameters: [
		     ...
		     steps.string(name: 'dockerImage', value: "${params.dockerImage}" ),
