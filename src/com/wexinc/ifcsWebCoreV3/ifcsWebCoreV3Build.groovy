package com.wexinc.ifcsWebCoreV3

class ifcsWebCoreV3Build implements Serializable {
   ...
   // pipeline global properties
    ...
    ifcsWebCoreV3Build(steps, env, params, scm, currentBuild) { 
        this.steps = steps
	this.env = env
        this.params = params
        this.scm = scm
	...
	// Deploying maven artifact and docker image
        steps.sh "mvn -s ${maven_settings} -U -f pom.xml deploy -Psign-jars -DskipTests=true"
    }
  
    def cascade(ifcsRepos) {
        for (int i = 0; i < ifcsRepos.size(); i++) {  // trigger ifcs-server job and deploy on respective client environment
             steps.build(job: "${orgName}/${ifcsRepos[i]}"])
        }			  
    }
